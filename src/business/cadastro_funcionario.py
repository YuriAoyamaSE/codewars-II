...
class CadastroFuncionario():
    ...
    def __init__(self, nome: str, cpf: str, data_admissao: str, cargos_codigo: str, comissao=False):
        self.nome = nome
        self.cpf = cpf
    ...
    def inclusao(self) -> None:
        ...
        insert_query = "INSERT INTO codewars2.funcionarios(nome,cpf,data_admissao,cargos_codigo,comissao) VALUES (%s,%s,%s,%s,%s)"
        insert_record = (self.nome, self.cpf, self.data_admissao,
                         self.cargos_codigo, self.comissao)
        cursor.execute(insert_query, insert_record)
        salvar_commit(cnx)

        insert_query = f"SELECT matricula FROM codewars2.funcionarios WHERE cpf = {self.cpf}"
